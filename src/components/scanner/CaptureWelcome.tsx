  ...
  return <div className="capture-home">
    ...
    <section className="capture-hero">
      ...
      <div className="capture-visual" onDrop={onDrop} onDragOver={e => e.preventDefault()}>
        ...
        <div className="sample-card"><div className="sample-brand"><ScanLine size={18} /><span>CARDSNAP CAPTURE</span></div><div className="sample-person"><strong>Daniel Rahman</strong><span>Business Development Manager</span></div><div className="sample-details"><span>daniel.rahman@example.com</span><span>+1 (555) 284-7712</span></div><div className="card-flight-path" /></div>
    ...
    <section className="connection-strip"><div><Globe2 size={21} /><span>Reviewed contact records.<small>Across conversations. Across your network.</small></span></div><span className="strip-caption">CARDSNAP BY V71</span><svg viewBox="0 0 160 32" aria-hidden="true"><path d="M0 27 C60 27 100 24 155 3" /><path d="m144 3 11 0-6 9" /></svg></section>
    <section className="workspace-grid">
      ...
      <button className="contacts-panel" onClick={onContacts}><div className="contacts-panel-top"><span>YOUR CONTACT COLLECTION</span><ArrowUpRight size={20} /></div><div className="contact-total">{String(count).padStart(2, '0')}<span>reviewed {count === 1 ? 'contact' : 'contacts'}</span></div><div className="contacts-panel-bottom"><span>{count ? 'Your next conversation starts here.' : 'Every great partnership starts somewhere.'}</span><ArrowRight size={17} /></div></button>
    </section>
    <footer className="workspace-footer"><span>CARDSNAP BY V71 <span className="footer-dot">/</span> CONTACT REVIEW</span><span>Business Card Capture & Contact Review</span></footer>
  </div>;
}
